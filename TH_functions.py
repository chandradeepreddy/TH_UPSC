...
def TH_Article_Content_Extractor(DayURL):
	""" 
	...
	"""
	try:
		title =BeautifulSoup(urllib2.urlopen(DayURL)).find_all("h1",{"class":re.compile("title")})[0].text.strip().encode('utf-8')
	except IndexError:
		title = ''

	try:
		caption =BeautifulSoup(urllib2.urlopen(DayURL)).find_all("h2",{"class":re.compile("intro")})[0].text.strip().encode('utf-8')
	except IndexError:
		caption = ''

	try:
		body =BeautifulSoup(urllib2.urlopen(DayURL)).find_all("div",{"id":re.compile("content-body-*")})[0].text.strip().encode('utf-8')
	except IndexError:
		body = ''

	try:
		imgUrl =re.sub('FREE_215','FREE_660',BeautifulSoup(urllib2.urlopen(DayURL)).find_all("img",{"class":re.compile("media-object adaptive placeholder lead-img")})[0]['data-proxy-image'])
	except IndexError:
		imgUrl = ''

	try:
		author = BeautifulSoup(urllib2.urlopen(DayURL)).find_all("a",{"class":re.compile("auth-nm lnk|auth-nm no-lnk")})[0].text.strip().encode('utf-8')
	except IndexError:
		author = ''

	try:
		tmstmp = BeautifulSoup(urllib2.urlopen(DayURL)).find_all("div",{"class":re.compile("ut-container")})[0].text.strip().encode('utf-8')
	except IndexError:
		tmstmp = ''
